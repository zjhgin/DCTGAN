...
def get_wav(in_channels, pool=True):
    harr_wav_L = 1 / np.sqrt(2) * np.ones((1, 2)) #哈尔小波变换低通滤波器
    harr_wav_H = 1 / np.sqrt(2) * np.ones((1, 2)) #高通滤波器
    harr_wav_H[0, 0] = -1 * harr_wav_H[0, 0] #调整高通滤波
    harr_wav_LL = np.transpose(harr_wav_L) * harr_wav_L #计算小波系数

    harr_wav_HH = np.transpose(harr_wav_H) * harr_wav_H
    filter_LL = torch.from_numpy(harr_wav_LL).unsqueeze(0) #转换为张量
    ...
    LL = net(in_channels, in_channels*2, #初始化卷积层
    ...
    HH = net(in_channels, in_channels*2,
             kernel_size=2, stride=2, padding=0, bias=False,
             groups=in_channels)
    LL.weight.requires_grad = False #False时权重参数设置为不可训练，梯度计算关闭，该参数不参与反向计算梯度

    HH.weight.requires_grad = False
    LL.weight.data = filter_LL.float().unsqueeze(0).expand(in_channels*2, -1, -1, -1) #将权重参数设置为小波系数
...
class WavePool(nn.Module):
    def __init__(self, in_channels):
        ...
        self.LL,self.HH = get_wav(in_channels)
...
#        self.Unpool
    ...
    def forward(self, x, skips):
        ...
        #HH1 = HH1.view(-1,3,c, h, w).mean(dim=1)
        ...
        # c, h, w = HH2.size()[-3:]
        # HH2 = HH2.view(-1, 3, c, h,w).mean(dim=1)
...
#        original3 = skips['conv2_1'].view(8, 3, c, h, w).mean(dim=1)
        c, h, w = HH3.size()[-3:]
        HH3 = HH3.view(-1, 3, c, h,w).mean(dim=1)
        x_deconv4 = self.recon_block1(x3, HH3, original2)
